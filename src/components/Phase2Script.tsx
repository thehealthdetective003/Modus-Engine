...
export function Phase2Script({state,setState}:Props){
 ...
 return <div className="space-y-6">
  ...
  {!!state.plannedScenes.length&&<div className="border rounded-md p-3 space-y-2"><div className="text-[10px] uppercase text-muted-foreground">Automatic documentary plan</div><div className="flex flex-wrap gap-2"><Badge variant="default">Operational: {planSummary.operationalScenes}</Badge><Badge variant="outline">Opening operational: {planSummary.openingOperationalScenes}</Badge>{planSummary.firstOperationalScene&&<Badge variant="outline">First: scene {planSummary.firstOperationalScene}</Badge>}</div><div className="flex flex-wrap gap-2">{planSummary.families.map(([k,v])=><Badge key={k} variant="secondary">{k.replaceAll('_',' ')}: {v}</Badge>)}</div><div className="flex flex-wrap gap-2">{planSummary.treatments.map(([k,v])=><Badge key={k} variant="outline">{k.replace('_T2V','').replaceAll('_',' ')}: {v}</Badge>)}</div></div>}
